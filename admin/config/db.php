...
class DB {
    ...
    public function verfiyUser($u,$p){
      ...
      $sql = "SELECT password FROM users WHERE username='".htmlspecialchars(strip_tags($u))."'";
      $query = $this->db->prepare($sql);
      $query->execute();
      if($query->rowCount() > 0){
        $verify=$query->fetchAll(PDO::FETCH_ASSOC);
        $verify = PHPassLib\Hash\BCrypt::verify($p, $verify[0]['password']);
    ...
    {
      $sql = "UPDATE settings SET s_value=s_value + 1 WHERE s_key = 'visitors'; ";
      $query = $this->db->prepare($sql);
      $query->execute();
      $sql = "SELECT s_value from settings WHERE s_key='visitors';";
      $query = $this->db->prepare($sql);
      $query->execute();
      $visitors = $query->fetchAll(PDO::FETCH_ASSOC);
      return $visitors;
    }
}
