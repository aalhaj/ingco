  ...
  angular
    ...
    function UserController($scope, $http) {
      ...
      $scope.saveUser = function(type){
          if($scope.tempUserData.password == $scope.tempUserData.password2){
            delete $scope.tempUserData.password2;
          }else {
            return false;
          }
          var data = $.param({
          ...
          $http.post("action.php", data, config).then(function(response){
              if(response.data.status == 'OK'){
                  if(type == 'edit'){
                      ...
                      $scope.users[$scope.index].username = $scope.tempUserData.username;
